...
def route_task(task_description):
    ...
    if any(kw in task_description for kw in ["pandas", "dataframe", "tabular", "etl"]):
        ...
        kw in task_description for kw in ["vector search", "ann", "embedding", "knn"]
    ):
        return "hipvs-ann"
    elif any(
        kw in task_description
        for kw in ["graph", "pagerank", "community detection", "bfs"]
    ):
        return "hipgraph-analytics"
    elif any(
        kw in task_description
        for kw in ["clustering", "linear algebra", "primitive", "math"]
    ):
        return "hipraft-primitives"
    elif any(
        kw in task_description for kw in ["oom", "memory", "fragmentation", "pool"]
    ):
