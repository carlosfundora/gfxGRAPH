...
- Entrypoints: `python/hipgraph_bridge/__init__.py` and native C++ `src/init.cpp`.
- Data Flow: PyTorch CUDA Graph API calls -> Python Monkey-patch (`BridgedCUDAGraph`) -> Either Native C++ Gap Bridges (`libhipgraph_bridge.so`) or Fallback.
- Orchestration Layers: Python monkey-patching module.
- Execution Engine: Direct wrapper around HIP graphs + specific `.hip` kernels for unsupported ops.
- Architecture Class: Framework wrapper / adapter.
- Concurrency Model: Synchronous pipeline.
...
| **DX / Onboarding** | 9 | 3 | 7 | gfxGRAPH is `import gfxgraph; gfxgraph.enable()`; MIGraphX requires `rbuild`. |
